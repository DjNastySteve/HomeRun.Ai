...
batter_metrics = stat_df[["player_name_clean", "barrel_batted_rate", "avg_best_speed", "hard_hit_percent"]].copy()
...
if df.empty:
    st.error("❌ No player lineup data was retrieved.")
    st.stop()
...
for pid in df["PitcherID"].dropna().unique():
    try:
        ...
        hr_per_9 = float(stats_data["stats"][0]["splits"][0]["stat"].get("hrPer9", 1.1))
        handedness = profile_data.get("people", [{}])[0].get("pitchHand", {}).get("code", "R")
        pitcher_stats[pid] = {"HR/9": round(hr_per_9, 2), "Handedness": handedness}
    except:
        pitcher_stats[pid] = {"HR/9": np.random.uniform(0.8, 2.2), "Handedness": "R"}
